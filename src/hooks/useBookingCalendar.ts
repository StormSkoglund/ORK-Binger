...
export function useBookingCalendar(options: UseBookingCalendarOptions = {}) {
  const {
    guestMode = false,
    guestName,
    guestEvents = [],
  ...
  const [dbEvents, setDbEvents] = useState<BookingEvent[]>([]);
  const [toasts, setToasts] = useState<ToastItem[]>([]);
  const [modalEvent, setModalEvent] = useState<ModalEvent | null>(null);
  const [supabaseError, setSupabaseError] = useState<string | null>(null);

  const events = [...dbEvents, ...guestEvents];
  ...
  function updateLocalBooking(
    ...
    const isGuestBooking = guestEvents.some((event) => event.id === id);
    if (isGuestBooking && updateGuestBooking) {
      updateGuestBooking(id, updated);
      return;
    }
    setDbEvents((prev) =>
  ...
  function removeLocalBooking(id: string) {
    const isGuestBooking = guestEvents.some((event) => event.id === id);
    if (isGuestBooking && removeGuestBooking) {
  ...
  async function handleEventReceive(info: any) {
    ...
    if (isRangeBooked(start, end)) {
      pushToast({
        ...
        message:
          "Time slot overlaps an existing booking — vennligst velg et annet tidspunkt.",
      });
      info.revert();
      return;
    }

    const usingLocalBooking = guestMode || !isSupabaseConfigured;
    if (usingLocalBooking) {
      ...
      event.setProp("id", localId);
      event.setExtendedProp("isGuest", true);
      pushToast({
        id: `guest-added-${localId}`,
        message: `${event.title} ble lagt til som gjestebooking.`,
        actionLabel: "Angre",
        onAction: () => removeLocalBooking(localId),
      });
      return;
    }
  ...
  function eventAllow(dropInfo: any) {
    ...
    const end =
      ...
      (start ? new Date(start.getTime() + 60 * 60 * 1000) : null);
    const draggingEvent = dropInfo.event;
    if (!start || !end) return false;

    const isGuestEvent = !!(
      draggingEvent?.extendedProps as Record<string, unknown>
    )?.isGuest;

    if (draggingEvent && !isGuestEvent && guestMode) {
      return false;
    }

    return !isRangeBooked(start, end, draggingEvent?.id as string);
  }
  ...
  async function handleEventDrop(info: any) {
    ...
    if (guestMode && !isGuestEvent) {
      pushToast({
        ...
        message: "Ekte bookinger kan ikke flyttes i demoen.",
      });
      info.revert();
      return;
    }
  ...
  async function deleteBooking(id: string) {
    ...
    if (isGuestBooking || !isSupabaseConfigured) {
      removeLocalBooking(id);
      pushToast({
        id: `guest-del-${id}`,
        message: "Din gjestebooking er fjernet.",
      });
      setModalEvent(null);
      return;
    }
  ...
  async function rescheduleBooking(
    ...
    if (isGuestBooking || !isSupabaseConfigured) {
      ...
      pushToast({
        id,
        message: `Oppdatert gjestebooking for ${id}.`,
      });
      return;
    }
